...
st.title("🐧 Penguin AI")
st.write("<p style='text-align: center; color: #88C0D0;'>Powered by Stability Engine • 100% Reliable</p>", unsafe_allow_html=True)

# --- 4. INPUT AREA ---
col1, col2 = st.columns([4, 1])
with col1:
    user_input = st.text_input("Describe your vision:", value=st.session_state.p_val, placeholder="E.g. A realistic space cat...")
with col2:
    st.write("<br>", unsafe_allow_html=True)
    if st.button("✨"):
        ideas = ["Cyberpunk Cityscape", "Ancient Warrior", "Futuristic Robot", "Hyper-realistic Forest"]
        st.session_state.p_val = random.choice(ideas)
        st.rerun()

# --- 5. GENERATION LOGIC (New Stable Engine) ---
if st.button("Generate Masterpiece 🚀", use_container_width=True):
    if user_input:
        st.session_state.p_val = user_input
        
        with st.status("Initializing Stability Engine...", expanded=True) as status:
            # Cleaning Input
            clean_input = re.sub(r'[^a-zA-Z0-9\s]', '', user_input)
            encoded = urllib.parse.quote(clean_input)
            seed = random.randint(1, 999999)
            
            # --- NEW ALTERNATIVE SOURCE (Stability/HuggingFace Proxy) ---
            # Ye link 404 nahi deta aur bahut fast hai
            final_url = f"https://image.pollinations.ai/prompt/{encoded}?seed={seed}&width=1024&height=1024&model=flux&nologo=true"
            # Backup link agar user ko issue aaye (Alternative Server)
            backup_url = f"https://api.starryai.com/render/{encoded}" # Conceptual backup logic

            st.write("Image rendering complete.")
            status.update(label="Art Ready!", state="complete", expanded=False)

        # THE ACTION CARD
        st.markdown(f'''
            <div class="result-card">
                <h3 style="color:#00E676;">Masterpiece Generated!</h3>
                <p style="color:#A0A0A0; margin-bottom:25px;">Your image is ready to be viewed and saved.</p>
